	...
	<body>
		...
		<div id="show-message" class="content scaffold-show" role="main">
			...
			<ol class="property-list message">
				...
				<g:if test="${messageInstance?.subscriber}">
				<li class="fieldcontain">
					<span id="subscriber-label" class="property-label"><g:message code="message.subscriber.label" default="Subscriber" /></span>
					
						<span class="property-value" aria-labelledby="subscriber-label"><g:link controller="subscriber" action="show" id="${messageInstance?.subscriber?.id}">${messageInstance?.subscriber?.encodeAsHTML()}</g:link></span>
